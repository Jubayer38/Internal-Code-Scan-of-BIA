...
{
    public class SIMReplacementOrderRequestValidatorAttribute : Attribute, IAsyncActionFilter
    {
        public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
        {
            var request = context.HttpContext.Request;
            var errors = new List<string>();
            SimReplacementRequestModel? model = null;
            ...
            if (isForm)
            {
                model = context.ActionArguments
                              .Values
                              .FirstOrDefault(v => v is SimReplacementRequestModel) as SimReplacementRequestModel;
            }
            else
            {
                ...
                try
                {
                    ...
                    model = JsonSerializer.Deserialize<SimReplacementRequestModel>(bodyStr, new JsonSerializerOptions
                    {
        ...
        private void ValidateModel(SimReplacementRequestModel model, List<string> errors)
        {
