...
{
    public class ValidateMSISDNValidatorModelAttribute : Attribute, IAsyncActionFilter
    {
        public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
        {
            var request = context.HttpContext.Request;
            var errors = new List<string>();
            UnpairedMSISDNCheckRequest? model = null;
            ...
            if (isForm)
            {
                model = context.ActionArguments
                              .Values
                              .FirstOrDefault(v => v is UnpairedMSISDNCheckRequest) as UnpairedMSISDNCheckRequest;
            }
            else
            {
                ...
                try
                {
                    ...
                    model = JsonSerializer.Deserialize<UnpairedMSISDNCheckRequest>(bodyStr, new JsonSerializerOptions
                    {
        ...
        private void ValidateModel(UnpairedMSISDNCheckRequest model, List<string> errors)
        {
            ...
            if (model.channel_id == 0 || model.channel_id < 0)
                errors.Add("Technical error!!! Channel Id is required, please resubmit the request.");
