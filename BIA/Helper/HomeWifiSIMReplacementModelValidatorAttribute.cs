...
{
    public class HomeWifiSIMReplacementModelValidatorAttribute : Attribute, IAsyncActionFilter
    {
        public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
        {
            var request = context.HttpContext.Request;
            var errors = new List<string>();
            SimreplacementValidationModel? model = null;
            ...
            if (isForm)
            {
                model = context.ActionArguments
                              .Values
                              .FirstOrDefault(v => v is SimreplacementValidationModel) as SimreplacementValidationModel;
            }
            else
            {
                ...
                try
                {
                    ...
                    model = JsonSerializer.Deserialize<SimreplacementValidationModel>(bodyStr, new JsonSerializerOptions
                    {
        ...
        private void ValidateModel(SimreplacementValidationModel model, List<string> errors)
        {
