...
{
    public class SIMReplacementModelValidatorAttribute : Attribute, IAsyncActionFilter
    {
        public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
        {
            var request = context.HttpContext.Request;
            var errors = new List<string>();
            IndividualSIMReplsMSISDNCheckRequest? model = null;
            ...
            if (isForm)
            {
                model = context.ActionArguments
                              .Values
                              .FirstOrDefault(v => v is IndividualSIMReplsMSISDNCheckRequest) as IndividualSIMReplsMSISDNCheckRequest;
            }
            else
            {
                ...
                try
                {
                    ...
                    model = JsonSerializer.Deserialize<IndividualSIMReplsMSISDNCheckRequest>(bodyStr, new JsonSerializerOptions
                    {
        ...
        private void ValidateModel(IndividualSIMReplsMSISDNCheckRequest model, List<string> errors)
        {
