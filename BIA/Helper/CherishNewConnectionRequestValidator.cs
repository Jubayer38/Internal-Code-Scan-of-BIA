...
{
    public class CherishNewConnectionRequestValidator : Attribute, IAsyncActionFilter
    {
        public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
        {
            var request = context.HttpContext.Request;
            var errors = new List<string>();
            CherishNewConnectionRequestModel? model = null;
            ...
            if (isForm)
            {
                model = context.ActionArguments
                              .Values
                              .FirstOrDefault(v => v is CherishNewConnectionRequestModel) as CherishNewConnectionRequestModel;
            }
            else
            {
                ...
                try
                {
                    ...
                    model = JsonSerializer.Deserialize<CherishNewConnectionRequestModel>(bodyStr, new JsonSerializerOptions
                    {
        ...
        private void ValidateModel(CherishNewConnectionRequestModel model, List<string> errors)
        {
            ...
            if (string.IsNullOrWhiteSpace(model.Selected_category))
                errors.Add("Technical error!!! Number Category is required, please resubmit the request.");
        }
    }
}
