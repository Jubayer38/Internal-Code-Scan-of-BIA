...
{
    public class SIMReplacementPOCModelValidatorAttribute : Attribute, IAsyncActionFilter
    {
        public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
        {
            var request = context.HttpContext.Request;
            var errors = new List<string>();
            CorporateMSISDNCheckRequest? model = null;
            ...
            if (isForm)
            {
                model = context.ActionArguments
                              .Values
                              .FirstOrDefault(v => v is CorporateMSISDNCheckRequest) as CorporateMSISDNCheckRequest;
            }
            else
            {
                ...
                try
                {
                    ...
                    model = JsonSerializer.Deserialize<CorporateMSISDNCheckRequest>(bodyStr, new JsonSerializerOptions
                    {
        ...
        private void ValidateModel(CorporateMSISDNCheckRequest model, List<string> errors)
        {
